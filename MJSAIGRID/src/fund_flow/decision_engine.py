...
class FundFlowDecisionEngine:
        ...
        self.deepseek_router = DeepSeekWeightRouter(
            {
                ...
                "deepseek_ai": ai_cfg,
                "logging": self.config.get("logging", {}) if isinstance(self.config.get("logging"), dict) else {},
            }
        )
    ...
    def assess_open_direction(
        self,
        symbol: str,
        market_flow_context: Dict[str, Any],
        allow_entry_window: bool = True,
    ) -> Dict[str, Any]:
        """
        仅输出开仓方向判断，供移动网格/方向性挂单复用。

        这个接口不会生成 BUY/SELL/CLOSE 操作，只暴露方向偏置：
        - LONG_ONLY
        - SHORT_ONLY
        - BOTH
        """
        ...
        guide_direction = str(
            regime_info.get("guide_direction", regime_info.get("direction", "BOTH"))
        ).upper()
        if guide_direction not in {"LONG_ONLY", "SHORT_ONLY", "BOTH"}:
            guide_direction = "BOTH"

        override_mode = self._get_symbol_side_override(symbol)
        blocked_by_override = False
        if override_mode == "LONG_ONLY" and guide_direction == "SHORT_ONLY":
            guide_direction = "BOTH"
            blocked_by_override = True
        elif override_mode == "SHORT_ONLY" and guide_direction == "LONG_ONLY":
            guide_direction = "BOTH"
            blocked_by_override = True

        blocked_by_window = not bool(allow_entry_window)
        effective_direction = "BOTH" if blocked_by_window else guide_direction
