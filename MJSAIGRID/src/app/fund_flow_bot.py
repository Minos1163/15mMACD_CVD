"""Fund-flow compatible trading runtime.

This runtime now supports two execution modes:
- FUND_FLOW: 旧资金流开平仓执行
- MOVING_FUTURES_GRID: 资金流仅判断开仓方向，执行层改为移动合约网格
"""
...
from src.grid_trading.grid_engine import GridEngine
from src.grid_trading.models import (
    AIDecision as UnifiedAIDecision,
    AccountBalance as UnifiedAccountBalance,
    GridConfig,
    GridMode as UnifiedGridMode,
    GridType,
    MarketState as UnifiedMarketState,
    OrderRequest as UnifiedOrderRequest,
    OrderSide as UnifiedOrderSide,
    OrderType as UnifiedOrderType,
    Position as UnifiedPosition,
    PositionSide as UnifiedPositionSide,
    StrategyState as UnifiedStrategyState,
    StrategyStatus as UnifiedStrategyStatus,
    TimeInForce as UnifiedTimeInForce,
)
from src.risk.risk_controller import RiskAction as UnifiedRiskAction
from src.risk.risk_controller import RiskCheckResult, RiskController
try:
...
class TradingBot:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.strategy_mode: str = "FUND_FLOW"
        self._moving_grid_enabled: bool = False
        self._grid_anchor_state: Dict[str, Dict[str, Any]] = {}
        ...
        self.risk_controller = RiskController(self._build_unified_risk_config())
        ...
        self._kline_debug_jsonl_name = "kline_debug_utc.jsonl"
        self._kline_debug_csv_name = "kline_debug_utc.csv"
        ...
        self._startup_risk_checked: bool = False
        self._startup_risk_allowed: bool = False
        self._startup_risk_reason: str = ""
        self._last_unified_risk_result_by_symbol: Dict[str, Dict[str, Any]] = {}
    ...
    def _refresh_strategy_mode(self) -> None:
        mode = str(self.config.get("strategy", {}).get("mode", "FUND_FLOW") or "FUND_FLOW").upper()
        self.strategy_mode = mode
        self._moving_grid_enabled = mode in self.MOVING_GRID_MODES

    def _build_unified_risk_config(self) -> GridConfig:
        ...
        grid_cfg = self.config.get("grid", {}) if isinstance(self.config, dict) else {}
        ...
        leverage = int(
            self._to_float(
                grid_cfg.get("leverage", trading_cfg.get("default_leverage", trading_cfg.get("min_leverage", 2))),
                2.0,
            )
        )
        config = GridConfig(
            symbol=str(symbols[0] if symbols else "DOGEUSDT"),
            capital=max(1.0, self._to_float(ff_cfg.get("capital", trading_cfg.get("capital", 100.0)), 100.0)),
            leverage=max(1, leverage),
            grid_mode=UnifiedGridMode.NEUTRAL,
            grid_type=GridType(str(grid_cfg.get("grid_type", "geometric")).lower()),
            price_lower=0.0,
            price_upper=0.0,
            grid_count=max(2, int(self._to_float(grid_cfg.get("grid_count", 12), 12))),
            active_order_count=max(1, int(self._to_float(grid_cfg.get("active_order_count", 6), 6))),
            moving_grid_enabled=bool(grid_cfg.get("moving_grid_enabled", self._moving_grid_enabled)),
            max_drawdown_threshold=_ratio(
                ff_cfg.get(
                    "max_drawdown_threshold",
                    risk_cfg.get("max_daily_loss_percent", 5),
                ),
                0.12,
            ),
            daily_loss_limit=_ratio(
                risk_cfg.get("max_daily_loss_percent", ff_cfg.get("daily_loss_limit", 5)),
                0.05,
            ),
            max_consecutive_losses=max(1, int(self._to_float(risk_cfg.get("max_consecutive_losses", 2), 2))),
            consecutive_loss_cooldown=max(
                0,
                int(self._to_float(risk_cfg.get("consecutive_loss_cooldown_seconds", 1800), 1800)),
            ),
            daily_loss_cooldown=max(
                0,
                int(self._to_float(risk_cfg.get("daily_loss_cooldown_seconds", 8 * 3600), 8 * 3600)),
            ),
            max_margin_usage=min(
                1.0,
                max(
                    0.05,
                    _ratio(
                        ff_cfg.get(
                            "max_margin_usage",
                            1.0 - _ratio(trading_cfg.get("reserve_percent", 20), 0.2),
                        ),
                        0.8,
                    ),
                ),
            ...
            max_position_ratio=min(
                ...
                max(
                    0.05,
                    _ratio(
                        trading_cfg.get("max_position_percent", ff_cfg.get("max_open_portion", 100)),
                        1.0,
                    ),
                ),
            ),
            max_leverage=max(1, int(self._to_float(trading_cfg.get("max_leverage", ff_cfg.get("max_leverage", 2)), 2))),
            abnormal_market_pause_enabled=bool(
                ff_cfg.get("extreme_volatility_cooldown_enabled", risk_cfg.get("abnormal_market_pause_enabled", True))
            ),
        )
        return config

    def _refresh_unified_risk_controller(self) -> None:
        previous = getattr(self, "risk_controller", None)
        controller = RiskController(self._build_unified_risk_config())
        if isinstance(previous, RiskController):
            controller.current_risk_level = previous.current_risk_level
            controller.pause_start_time = previous.pause_start_time
            controller.cooldown_end_time = previous.cooldown_end_time
            controller.consecutive_losses = previous.consecutive_losses
            controller.last_loss_time = previous.last_loss_time
            controller.daily_pnl = previous.daily_pnl
            controller.daily_reset_time = previous.daily_reset_time
            controller.abnormal_market_detected = previous.abnormal_market_detected
            controller.last_abnormal_check_time = previous.last_abnormal_check_time
        self.risk_controller = controller
        self._sync_unified_risk_state()
        self._startup_risk_checked = False
        self._startup_risk_allowed = False
        self._startup_risk_reason = ""

    def _sync_unified_risk_state(self, account_summary: Optional[Dict[str, Any]] = None) -> None:
        controller = getattr(self, "risk_controller", None)
        if not isinstance(controller, RiskController):
            return
        controller.consecutive_losses = int(self._consecutive_losses or 0)
        if isinstance(self._cooldown_expires, datetime):
            controller.pause_start_time = datetime.now(self._cooldown_expires.tzinfo) if self._cooldown_expires.tzinfo else datetime.now()
            controller.cooldown_end_time = self._cooldown_expires
        else:
            controller.pause_start_time = None
            controller.cooldown_end_time = None
        if isinstance(self._daily_open_date, str) and self._daily_open_date:
            try:
                controller.daily_reset_time = datetime.fromisoformat(f"{self._daily_open_date}T00:00:00")
            except Exception:
                pass
        if isinstance(account_summary, dict):
            equity = self._to_float(account_summary.get("equity"), 0.0)
            open_equity = self._to_float(self._daily_open_equity, 0.0)
            if equity > 0 and open_equity > 0:
                controller.daily_pnl = equity - open_equity

    def _current_drawdown_ratio(self, account_summary: Optional[Dict[str, Any]]) -> float:
        if not isinstance(account_summary, dict):
            return 0.0
        equity = self._to_float(account_summary.get("equity"), 0.0)
        peak = self._to_float(self._peak_equity, 0.0)
        if equity <= 0 or peak <= 0:
            return 0.0
        return max(0.0, min(1.0, (peak - equity) / peak))

    def _to_unified_account_balance(self, account_summary: Optional[Dict[str, Any]]) -> UnifiedAccountBalance:
        account_summary = account_summary if isinstance(account_summary, dict) else {}
        return UnifiedAccountBalance(
            asset="USDT",
            wallet_balance=self._to_float(account_summary.get("total_balance"), 0.0),
            available_balance=self._to_float(account_summary.get("available_balance"), 0.0),
            margin_balance=self._to_float(account_summary.get("equity"), 0.0),
            unrealized_pnl=self._to_float(account_summary.get("total_unrealized_pnl"), 0.0),
        )

    def _to_unified_positions(self, positions_payload: Optional[Dict[str, Dict[str, Any]]] = None) -> List[UnifiedPosition]:
        payload = positions_payload if isinstance(positions_payload, dict) else self._position_snapshot_by_symbol(
            ConfigLoader.get_trading_symbols(self.config)
        )
        out: List[UnifiedPosition] = []
        for symbol, position in (payload or {}).items():
            if not isinstance(position, dict):
                continue
            side = str(position.get("side", "")).upper()
            if side not in ("LONG", "SHORT"):
                continue
            pos_side = UnifiedPositionSide.LONG if side == "LONG" else UnifiedPositionSide.SHORT
            out.append(
                UnifiedPosition(
                    symbol=str(symbol).upper(),
                    side=pos_side,
                    quantity=abs(self._to_float(position.get("amount"), 0.0)),
                    ...
                    leverage=max(1, int(self._to_float(position.get("leverage"), 1.0))),
                    liquidation_price=self._to_float(position.get("liquidation_price"), 0.0),
                )
            )
        return out

    def _to_unified_strategy_state(
        self,
        *,
        symbol: str,
        current_price: float,
    ...
    ) -> UnifiedStrategyState:
        side = str((position or {}).get("side") or decision_side or "").upper()
        if side == "LONG":
            mode = UnifiedGridMode.LONG
        elif side == "SHORT":
            mode = UnifiedGridMode.SHORT
        else:
            mode = UnifiedGridMode.NEUTRAL
        leverage = max(
            1,
            int(
                self._to_float(
                    (position or {}).get(
                        "leverage",
                            self.config.get("grid", {}).get(
                                "leverage",
                                self.config.get("trading", {}).get("default_leverage", 2),
                            ),
                        ),
                    2.0,
                )
        ...
        return UnifiedStrategyState(
            status=UnifiedStrategyStatus.RUNNING,
            current_mode=mode,
            current_leverage=leverage,
            current_price=current_price,
            total_pnl=self._to_float((account_summary or {}).get("total_unrealized_pnl"), 0.0),
            daily_pnl=self._to_float(
                self._to_float((account_summary or {}).get("equity"), 0.0) - self._to_float(self._daily_open_equity, 0.0),
                0.0,
            ),
            max_drawdown=self._current_drawdown_ratio(account_summary),
            consecutive_losses=int(self._consecutive_losses or 0),
        )

    def _to_unified_market_state(
        self,
        *,
        ...
        if volatility_blocked:
            return UnifiedMarketState.ABNORMAL
        md = getattr(decision, "metadata", None)
        md = md if isinstance(md, dict) else {}
        engine = str(md.get("engine") or md.get("regime") or "").upper()
        if engine == "RANGE":
            return UnifiedMarketState.RANGE
        if engine == "TREND":
            op = getattr(decision, "operation", None)
            if op == FundFlowOperation.BUY:
                return UnifiedMarketState.UPTREND
            if op == FundFlowOperation.SELL:
                return UnifiedMarketState.DOWNTREND
        direction = str(direction_mode or "").upper()
        if direction == "LONG":
            return UnifiedMarketState.UPTREND
        if direction == "SHORT":
            return UnifiedMarketState.DOWNTREND
        return UnifiedMarketState.UNKNOWN

    def _to_unified_ai_decision(self, decision: Optional[FundFlowDecision]) -> Optional[UnifiedAIDecision]:
        if decision is None:
            return None
        md = getattr(decision, "metadata", None)
        md = md if isinstance(md, dict) else {}
        ...
        if op == FundFlowOperation.BUY:
            mode = UnifiedGridMode.LONG
            market_state = UnifiedMarketState.UPTREND
        elif op == FundFlowOperation.SELL:
            mode = UnifiedGridMode.SHORT
            market_state = UnifiedMarketState.DOWNTREND
        else:
            mode = UnifiedGridMode.NEUTRAL
            market_state = UnifiedMarketState.UNKNOWN
        risk_score = int(max(0.0, min(100.0, self._to_float(md.get("risk_score", 50.0), 50.0))))
        if risk_score == 50:
            conflict_penalty = abs(self._to_float(md.get("risk_penalty"), 0.0))
            risk_score = int(max(0.0, min(100.0, 50.0 + conflict_penalty * 100.0)))
        return UnifiedAIDecision(
            market_state=market_state,
            recommended_mode=mode,
            recommended_leverage=max(1, int(self._to_float(getattr(decision, "leverage", 1), 1))),
            risk_score=risk_score,
            confidence=max(0.0, min(1.0, self._to_float(md.get("ds_confidence", md.get("confidence", 0.5)), 0.5))),
            reason=str(getattr(decision, "reason", "") or ""),
        )

    def _build_unified_order_request(
        self,
        *,
        symbol: str,
        side: str,
        ...
        order_side = UnifiedOrderSide.BUY if side_norm == "BUY" else UnifiedOrderSide.SELL
        position_side = UnifiedPositionSide.BOTH
        if not reduce_only:
            position_side = UnifiedPositionSide.LONG if side_norm == "BUY" else UnifiedPositionSide.SHORT
        return UnifiedOrderRequest(
            client_order_id=f"{source}_{symbol}_{side_norm}_{int(time.time() * 1000)}",
            symbol=symbol,
            side=order_side,
            position_side=position_side,
            order_type=UnifiedOrderType.LIMIT if price > 0 else UnifiedOrderType.MARKET,
            price=price if price > 0 else None,
            quantity=max(0.0, quantity),
            time_in_force=UnifiedTimeInForce.GTC,
            reduce_only=bool(reduce_only),
            strategy_id=self.strategy_mode,
            metadata={"source": source},
        )

    def _should_block_entries_from_unified_risk(self, result: Optional[RiskCheckResult]) -> bool:
        if not isinstance(result, RiskCheckResult):
            return False
        if result.allowed:
            return False
        return result.action in {
            UnifiedRiskAction.LIMIT_NEW_ORDER,
            UnifiedRiskAction.REDUCE_LEVERAGE,
            UnifiedRiskAction.REDUCE_POSITION,
            UnifiedRiskAction.CANCEL_NON_CRITICAL_ORDERS,
            UnifiedRiskAction.PAUSE_STRATEGY,
            UnifiedRiskAction.STOP_STRATEGY,
            UnifiedRiskAction.FORCE_CLOSE,
        }

    def _evaluate_unified_symbol_risk(
        self,
        *,
        symbol: str,
        current_price: float,
        ...
        position: Optional[Dict[str, Any]],
        decision: Optional[FundFlowDecision] = None,
        positions_payload: Optional[Dict[str, Dict[str, Any]]] = None,
        volatility_blocked: bool = False,
        direction_mode: Optional[str] = None,
        ...
        if not isinstance(controller, RiskController):
            return None
        self._sync_unified_risk_state(account_summary)
        try:
            result = controller.evaluate_risk(
                market_state=self._to_unified_market_state(
                    decision=decision,
                    volatility_blocked=volatility_blocked,
                    direction_mode=direction_mode,
                ),
                ai_decision=self._to_unified_ai_decision(decision),
                strategy_state=self._to_unified_strategy_state(
                    symbol=symbol,
                    current_price=current_price,
                    position=position,
                    account_summary=account_summary,
                    decision_side=direction_mode,
                ),
                account_balance=self._to_unified_account_balance(account_summary),
                positions=self._to_unified_positions(positions_payload),
                current_price=current_price,
            )
            self._last_unified_risk_result_by_symbol[str(symbol).upper()] = {
            ...
            print(f"⚠️ {symbol} 统一风控评估失败: {e}")
            return None

    def _check_unified_pre_order(
        self,
        *,
        symbol: str,
        ...
        self._sync_unified_risk_state(account_summary)
        try:
            return controller.pre_order_check(
            ...
            print(f"⚠️ {symbol} 统一下单前风控失败: {e}")
            return None

    def _ensure_startup_risk_check(self, force: bool = False) -> bool:
        if self._startup_risk_checked and not force:
            if not self._startup_risk_allowed and self._startup_risk_reason:
                print(f"⛔ 启动统一风控未通过: {self._startup_risk_reason}")
            return self._startup_risk_allowed

        controller = getattr(self, "risk_controller", None)
        if not isinstance(controller, RiskController):
            self._startup_risk_checked = True
            self._startup_risk_allowed = True
            self._startup_risk_reason = ""
            return True

        symbols = ConfigLoader.get_trading_symbols(self.config)
        symbol = str(symbols[0] if symbols else self.config.get("trading", {}).get("symbols", ["DOGEUSDT"])[0]).upper()
        account_summary = self.account_data.get_account_summary()
        if not isinstance(account_summary, dict):
            self._startup_risk_checked = True
            self._startup_risk_allowed = False
            self._startup_risk_reason = "account_summary_unavailable"
            print("⛔ 启动统一风控未通过: 账户信息不可用")
            return False
        ...
        try:
            ...
            print(f"⚠️ 启动统一风控获取价格失败 {symbol}: {e}")
        if current_price <= 0:
            self._startup_risk_checked = True
            self._startup_risk_allowed = False
            self._startup_risk_reason = "startup_price_unavailable"
            print(f"⛔ 启动统一风控未通过: {symbol} 当前价格不可用")
            return False

        config = self._build_unified_risk_config()
        self._sync_unified_risk_state(account_summary)
        ...
        self._startup_risk_checked = True
        self._startup_risk_allowed = bool(result.allowed)
        self._startup_risk_reason = str(result.reason or "")
        if result.allowed:
            print(
                "🛡️ 统一风控启动检查通过: "
                f"symbol={symbol} risk={result.risk_level.value} leverage={config.leverage}x"
            )
            return True
        print(
            "⛔ 启动统一风控未通过: "
            f"action={result.action.value} risk={result.risk_level.value} reason={result.reason}"
        )
        return False

    def _cycle_mode_label(self) -> str:
        return "MOVING_GRID" if self._moving_grid_enabled else "FUND_FLOW"
    ...
    def _reload_config_if_changed(self) -> bool:
        ...
        self._apply_network_env_from_config()
        self._init_fund_flow_modules()
        self._refresh_unified_risk_controller()
    ...
    def _resolve_kline_debug_jsonl_path_utc(self) -> str:
        return os.path.join(self.logs_dir, self._kline_debug_jsonl_name)

    def _resolve_kline_debug_csv_path_utc(self) -> str:
        return os.path.join(self.logs_dir, self._kline_debug_csv_name)

    def _position_snapshot_text(self, pos_obj: Any) -> str:
        if not isinstance(pos_obj, dict):
            return "FLAT:0"
        side = str(pos_obj.get("side") or pos_obj.get("positionSide") or "").upper()
        amt_raw = self._to_float(pos_obj.get("amount", pos_obj.get("positionAmt", 0.0)), 0.0)
        amt = abs(amt_raw)
        if side not in ("LONG", "SHORT"):
            if amt_raw > 0:
                side = "LONG"
        ...
        if amt <= 0:
            return "FLAT:0"
        entry_price = self._to_float(pos_obj.get("entry_price", pos_obj.get("entryPrice", 0.0)), 0.0)
        return f"{side}:{amt:.6f}@{entry_price:.8f}"

    def _latest_kline_snapshot(self, symbol: str, timeframe: str) -> Dict[str, Any]:
        tf = str(timeframe or "").strip().lower() or "15m"
        try:
            klines = self.client.get_klines(symbol=symbol, interval=tf, limit=1) or []
        except Exception:
            klines = []
        ...
        close_time = self._to_int(item[6], 0) if len(item) > 6 else 0
        return {
            "timeframe": tf,
            "open_time": self._to_int(item[0], 0),
            "close_time": close_time,
            "open": self._to_float(item[1], 0.0),
            "high": self._to_float(item[2], 0.0),
            "low": self._to_float(item[3], 0.0),
            "close": self._to_float(item[4], 0.0),
            "volume": self._to_float(item[5], 0.0),
        }

    def _append_kline_debug_event(self, payload: Dict[str, Any]) -> None:
        if not isinstance(payload, dict):
            return
        record = {"ts": datetime.now(timezone.utc).isoformat(), **payload}
        jsonl_path = self._resolve_kline_debug_jsonl_path_utc()
        with open(jsonl_path, "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False, default=str) + "\n")

        headers = [
            "时间(UTC)", "Cycle", "模式", "事件", "合约", "周期",
            "K线开盘时间", "K线收盘时间", "K线开", "K线高", "K线低", "K线收", "K线量",
            "现价", "决策", "状态", "原因", "触发类型",
            "方向", "指导方向", "指导分",
            "15m多", "15m空", "5m多", "5m空", "最终多", "最终空",
            "long_score", "short_score", "adx", "atr_pct",
            "cvd", "cvd_mom", "oi_delta", "funding", "depth", "imbalance", "liq_norm",
            "目标占比", "当前占比", "杠杆请求", "杠杆实际",
            "持仓前", "持仓后", "挂单数", "新增挂单", "撤销挂单",
            "参数快照JSON", "调试JSON",
        ]
        kline = record.get("kline") if isinstance(record.get("kline"), dict) else {}
        ...
        row = {
            "时间(UTC)": record.get("ts"),
            "Cycle": record.get("cycle"),
            "模式": record.get("mode"),
            "事件": record.get("event"),
            "合约": record.get("symbol"),
            "周期": kline.get("timeframe"),
            "K线开盘时间": kline.get("open_time"),
            "K线收盘时间": kline.get("close_time"),
            "K线开": kline.get("open"),
            "K线高": kline.get("high"),
            "K线低": kline.get("low"),
            "K线收": kline.get("close"),
            "K线量": kline.get("volume"),
            "现价": market.get("current_price"),
            "决策": record.get("action"),
            "状态": record.get("status"),
            "原因": record.get("reason"),
            "触发类型": record.get("trigger_type"),
            "方向": record.get("direction"),
            "指导方向": record.get("guide_direction"),
            "指导分": record.get("guide_score"),
            "15m多": scores.get("score_15m_long"),
            "15m空": scores.get("score_15m_short"),
            "5m多": scores.get("score_5m_long"),
            "5m空": scores.get("score_5m_short"),
            "最终多": scores.get("final_long"),
            "最终空": scores.get("final_short"),
            "long_score": scores.get("long_score"),
            "short_score": scores.get("short_score"),
            "adx": market.get("adx"),
            "atr_pct": market.get("atr_pct"),
            "cvd": market.get("cvd"),
            "cvd_mom": market.get("cvd_mom"),
            "oi_delta": market.get("oi_delta"),
            "funding": market.get("funding"),
            "depth": market.get("depth"),
            "imbalance": market.get("imbalance"),
            "liq_norm": market.get("liq_norm"),
            "目标占比": risk.get("target_portion"),
            "当前占比": risk.get("current_portion"),
            "杠杆请求": risk.get("requested_leverage"),
            "杠杆实际": risk.get("applied_leverage"),
            "持仓前": risk.get("position_pre"),
            "持仓后": risk.get("position_post"),
            "挂单数": risk.get("open_order_count"),
            "新增挂单": risk.get("placed_orders"),
            "撤销挂单": risk.get("cancelled_orders"),
            "参数快照JSON": json.dumps(record.get("params_snapshot", {}), ensure_ascii=False, separators=(",", ":"), default=str),
            "调试JSON": json.dumps(record.get("debug", {}), ensure_ascii=False, separators=(",", ":"), default=str),
        }
        csv_path = self._resolve_kline_debug_csv_path_utc()
    ...
    def _print_startup_summary(self) -> None:
        ...
        grid_cfg = self._moving_grid_config()
        symbols = ConfigLoader.get_trading_symbols(self.config)
        startup_cfg = self._startup_market_preload_config()
        print("=" * 66)
        print("🚀 交易机器人启动")
        print(f"📄 配置文件: {self.config_path}")
        print(f"🧠 运行模式: {self.strategy_mode}")
        print(f"📁 日志目录: {self.logs_dir}")
        print(f"🗂️ 分桶日志根目录(6H): {self.log_root_dir}")
        print(f"🧾 成交回报日志(UTC): {self._resolve_trade_fill_log_path_utc()}")
        print(f"🧪 K线调试JSONL(UTC): {self._resolve_kline_debug_jsonl_path_utc()}")
        print(f"🧪 K线调试CSV(UTC): {self._resolve_kline_debug_csv_path_utc()}")
        ...
        if self._moving_grid_enabled:
            print(
                "🧱 移动网格: "
                f"type={grid_cfg.get('grid_type')}, "
                f"count={grid_cfg.get('grid_count')}, "
                f"active={grid_cfg.get('active_order_count')}, "
                f"exit_active={grid_cfg.get('position_exit_order_count')}, "
                f"moving={grid_cfg.get('moving_grid_enabled')}, "
                f"neutral_action={grid_cfg.get('neutral_action')}"
            )
        print(
            "🧯 账户熔断: "
    ...
    def _apply_ma10_macd_entry_filter(self, symbol: str, decision: FundFlowDecision) -> FundFlowDecision:
        ...
        macd_trigger_pass_long = bool(
            md.get("macd_trigger_pass_long", md.get("confluence_macd_trigger_long", False))
        )
        macd_trigger_pass_short = bool(
            md.get("macd_trigger_pass_short", md.get("confluence_macd_trigger_short", False))
        )
        macd_early_pass_long = bool(
            md.get("macd_early_pass_long", md.get("confluence_macd_early_long", False))
        )
        macd_early_pass_short = bool(
            md.get("macd_early_pass_short", md.get("confluence_macd_early_short", False))
        )
    ...
    def _moving_grid_config(self) -> Dict[str, Any]:
        raw = self.config.get("grid", {})
        cfg = raw if isinstance(raw, dict) else {}
        ff_cfg = self.config.get("fund_flow", {}) or {}
        trading_cfg = self.config.get("trading", {}) or {}
        budget_portion = self._normalize_percent_to_ratio(
            cfg.get("budget_portion", ff_cfg.get("default_target_portion", 0.10)),
            0.10,
        )
        max_symbol_portion = self._normalize_percent_to_ratio(
            ff_cfg.get("max_symbol_position_portion", budget_portion),
            budget_portion,
        )
        leverage = int(self._to_float(cfg.get("leverage", trading_cfg.get("default_leverage", 2)), 2))
        min_leverage = int(self._to_float(trading_cfg.get("min_leverage", 1), 1))
        max_leverage = int(self._to_float(trading_cfg.get("max_leverage", max(leverage, 1)), max(leverage, 1)))
        leverage = max(min_leverage, min(max_leverage, leverage))
        grid_count = max(4, int(self._to_float(cfg.get("grid_count", 12), 12)))
        active_order_count = max(1, min(grid_count, int(self._to_float(cfg.get("active_order_count", 6), 6))))
        position_exit_order_count = max(
            1,
            min(grid_count, int(self._to_float(cfg.get("position_exit_order_count", active_order_count), active_order_count))),
        )
        neutral_action = str(cfg.get("neutral_action", "pause") or "pause").strip().lower()
        if neutral_action not in {"pause", "neutral"}:
            neutral_action = "pause"
        return {
            "enabled": bool(cfg.get("enabled", True)),
            "grid_type": str(cfg.get("grid_type", "arithmetic") or "arithmetic").strip().lower(),
            "grid_count": grid_count,
            "active_order_count": active_order_count,
            "position_exit_order_count": position_exit_order_count,
            "moving_grid_enabled": bool(cfg.get("moving_grid_enabled", True)),
            "auto_range_pct": self._normalize_percent_to_ratio(cfg.get("auto_range_pct", 0.02), 0.02),
            "min_range_pct": self._normalize_percent_to_ratio(cfg.get("min_range_pct", 0.006), 0.006),
            "max_range_pct": self._normalize_percent_to_ratio(cfg.get("max_range_pct", 0.05), 0.05),
            "atr_range_multiplier": max(0.0, self._to_float(cfg.get("atr_range_multiplier", 2.5), 2.5)),
            "recenter_threshold_pct": self._normalize_percent_to_ratio(
                cfg.get("recenter_threshold_pct", 0.003),
                0.003,
            ),
            "budget_portion": min(max_symbol_portion, budget_portion),
            "allow_scale_in_with_position": bool(cfg.get("allow_scale_in_with_position", True)),
            "available_balance_buffer": min(
                1.0,
                max(0.1, self._to_float(cfg.get("available_balance_buffer", 0.95), 0.95)),
            ),
            "position_min_tp_pct": self._normalize_percent_to_ratio(cfg.get("position_min_tp_pct", 0.002), 0.002),
            "neutral_action": neutral_action,
            "leverage": leverage,
        }

    def _evaluate_moving_grid_bias(
        self,
        symbol: str,
        ...
        if bool(confluence_cfg.get("enabled", True)):
            try:
                ...
                print(f"⚠️ {symbol} 移动网格方向特征注入失败: {e}")
        bias = self.fund_flow_decision_engine.assess_open_direction(
            symbol=symbol,
            market_flow_context=flow_context,
            allow_entry_window=allow_new_entries,
        )
        bias["flow_context"] = flow_context
        bias["flow_snapshot"] = flow_snapshot
        return bias

    def _moving_grid_direction_mode(self, bias: Dict[str, Any]) -> str:
        grid_cfg = self._moving_grid_config()
        direction = str(bias.get("direction", "BOTH") or "BOTH").upper()
        regime = str(bias.get("regime", "NO_TRADE") or "NO_TRADE").upper()
        if direction == "LONG_ONLY":
            return "LONG"
        if direction == "SHORT_ONLY":
            return "SHORT"
        if regime == "NO_TRADE" or grid_cfg.get("neutral_action") == "pause":
            return "PAUSE"
        return "NEUTRAL"

    def _list_open_entry_orders(self, symbol: str) -> List[Dict[str, Any]]:
        try:
    ...
    @staticmethod
    def _grid_order_signature(side: str, price: float, quantity: Optional[float] = None) -> str:
        sig = f"{str(side).upper()}@{float(price):.8f}"
        if quantity is None:
            return sig
        return f"{sig}#{float(quantity):.8f}"
    ...
    @staticmethod
    def _moving_grid_order_prefix(kind: str) -> str:
        return "MGX_" if str(kind).lower() == "exit" else "MGE_"

    def _make_moving_grid_client_order_id(self, *, symbol: str, side: str, price: float, kind: str) -> str:
        prefix = self._moving_grid_order_prefix(kind)
        side_code = "B" if str(side).upper() == "BUY" else "S"
        price_code = str(int(round(max(0.0, float(price)) * 1e8)))
        ts_code = str(int(time.time() * 1000) % 1_000_000)
        client_id = f"{prefix}{str(symbol).upper()}_{side_code}_{price_code}_{ts_code}"
        return client_id[:36]

    def _is_managed_moving_grid_exit_order(self, order: Dict[str, Any]) -> bool:
        client_id = self._order_client_id(order)
        return client_id.startswith(self._moving_grid_order_prefix("exit"))

    def _list_open_grid_exit_orders(self, symbol: str) -> List[Dict[str, Any]]:
        try:
            orders = self.client.get_open_orders(symbol) or []
        except Exception:
            return []
        ...
        for order in orders:
            if not isinstance(order, dict):
                continue
            if not self._is_managed_moving_grid_exit_order(order):
                continue
            is_reduce = bool(order.get("reduceOnly", False))
            is_close = bool(order.get("closePosition", False))
    ...
    def _build_moving_grid_levels(
        ...
        *,
        ...
        direction_mode: str,
        market_data: Dict[str, Any],
        existing_reference_orders: List[Dict[str, Any]],
        ...
        if current_price <= 0:
            return [], meta
        if direction_mode not in {"LONG", "SHORT"}:
            return [], meta

        trend_filter = market_data.get("trend_filter", {}) if isinstance(market_data, dict) else {}
        atr_pct = abs(self._to_float((trend_filter or {}).get("atr_pct"), 0.0))
        base_range_pct = self._normalize_percent_to_ratio(grid_cfg.get("auto_range_pct", 0.02), 0.02)
        atr_range_pct = atr_pct * self._to_float(grid_cfg.get("atr_range_multiplier"), 0.0)
        range_pct = max(base_range_pct, atr_range_pct)
        range_pct = max(
            self._normalize_percent_to_ratio(grid_cfg.get("min_range_pct", 0.006), 0.006),
            min(
                self._normalize_percent_to_ratio(grid_cfg.get("max_range_pct", 0.05), 0.05),
                range_pct,
            ),
        )
        anchor_state = self._grid_anchor_state.get(symbol, {})
        previous_anchor = self._to_float(anchor_state.get("anchor_price"), 0.0)
        previous_direction = str(anchor_state.get("direction_mode", "")).upper()
        recenter_threshold = self._normalize_percent_to_ratio(
            grid_cfg.get("recenter_threshold_pct", 0.003),
            0.003,
        )
        price_move = abs((current_price - previous_anchor) / previous_anchor) if previous_anchor > 0 else 0.0
        should_recenter = (
            (not bool(grid_cfg.get("moving_grid_enabled", True)))
            or previous_anchor <= 0
            or previous_direction != direction_mode
            or price_move >= recenter_threshold
            or not existing_reference_orders
        )
        anchor_price = current_price if should_recenter else previous_anchor
        self._grid_anchor_state[symbol] = {
            "anchor_price": anchor_price,
            "direction_mode": direction_mode,
            "updated_at": time.time(),
        }
        meta["anchor_price"] = anchor_price
        meta["range_pct"] = range_pct
        meta["recentered"] = should_recenter

        grid_type_name = str(grid_cfg.get("grid_type", "arithmetic")).upper()
        try:
            grid_type = GridType[grid_type_name]
        except Exception:
            grid_type = GridType.ARITHMETIC
        runtime_grid_cfg = GridConfig(
            symbol=symbol,
            leverage=int(grid_cfg.get("leverage", 2)),
            grid_type=grid_type,
            grid_count=int(grid_cfg.get("grid_count", 12)),
            active_order_count=int(grid_cfg.get("active_order_count", 6)),
            price_lower=anchor_price * (1.0 - range_pct),
            price_upper=anchor_price * (1.0 + range_pct),
        )
        levels = GridEngine(runtime_grid_cfg).generate_grid_levels(anchor_price)
        return levels, meta

    def _compute_moving_grid_targets(
        self,
        *,
        symbol: str,
        current_price: float,
        ...
        position: Optional[Dict[str, Any]],
        market_data: Dict[str, Any],
        existing_entry_orders: List[Dict[str, Any]],
    ) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        grid_cfg = self._moving_grid_config()
        meta: Dict[str, Any] = {"direction_mode": direction_mode}
        if current_price <= 0:
            return [], meta
        if direction_mode not in {"LONG", "SHORT"}:
            return [], meta
        position_side = str((position or {}).get("side", "")).upper()
        if position_side in {"LONG", "SHORT"} and position_side != direction_mode:
            meta["blocked_reason"] = f"opposite_position:{position_side}"
            return [], meta
        if position is not None and (not grid_cfg.get("allow_scale_in_with_position", True)):
            meta["blocked_reason"] = "position_exists_scale_in_disabled"
            return [], meta

        levels, runtime_meta = self._build_moving_grid_levels(
            symbol=symbol,
            current_price=current_price,
            direction_mode=direction_mode,
            market_data=market_data,
            existing_reference_orders=existing_entry_orders,
        )
        meta.update(runtime_meta)
        anchor_price = self._to_float(meta.get("anchor_price"), current_price)
        if direction_mode == "LONG":
            candidates = sorted(
                [lvl for lvl in levels if self._to_float(getattr(lvl, "price", 0.0), 0.0) < anchor_price],
                key=lambda lvl: abs(anchor_price - self._to_float(getattr(lvl, "price", 0.0), 0.0)),
            )
            side = "BUY"
        else:
            candidates = sorted(
                [lvl for lvl in levels if self._to_float(getattr(lvl, "price", 0.0), 0.0) > anchor_price],
                key=lambda lvl: abs(self._to_float(getattr(lvl, "price", 0.0), 0.0) - anchor_price),
            )
            side = "SELL"
        candidates = candidates[: int(grid_cfg.get("active_order_count", 6))]
        if not candidates:
            meta["blocked_reason"] = "no_grid_candidates"
            return [], meta

        total_balance = max(
            0.0,
            self._to_float(account_summary.get("equity"), self._to_float(account_summary.get("total_balance"), 0.0)),
        )
        available_balance = max(0.0, self._to_float(account_summary.get("available_balance"), 0.0))
        position_margin = max(0.0, self._to_float((position or {}).get("margin"), 0.0))
        budget_margin = total_balance * self._normalize_percent_to_ratio(grid_cfg.get("budget_portion", 0.10), 0.10)
        available_cap = available_balance * self._to_float(grid_cfg.get("available_balance_buffer"), 0.95)
        margin_cap = min(budget_margin, available_cap) if available_cap > 0 else budget_margin
        remaining_margin = max(0.0, margin_cap - position_margin)
        meta["budget_margin"] = budget_margin
        meta["remaining_margin"] = remaining_margin
        if remaining_margin <= 0:
            meta["blocked_reason"] = "no_remaining_margin"
            return [], meta

        leverage = max(1, int(grid_cfg.get("leverage", 2)))
        margin_per_order = remaining_margin / max(1, len(candidates))
        targets: List[Dict[str, Any]] = []
        for lvl in candidates:
            level_price = self._to_float(getattr(lvl, "price", 0.0), 0.0)
            if level_price <= 0:
                continue
            qty = (margin_per_order * leverage) / level_price
            try:
                qty = float(self.client.ensure_min_notional_quantity(symbol, qty, level_price))
                qty = float(self.client.format_quantity(symbol, qty))
            except Exception:
                qty = 0.0
            if qty <= 0:
                continue
            price = self.fund_flow_execution_router._format_price(symbol, level_price)
            targets.append({"side": side, "price": price, "quantity": qty})
        return targets, meta

    def _compute_position_exit_grid_targets(
        self,
        *,
        symbol: str,
        current_price: float,
        position: Optional[Dict[str, Any]],
        market_data: Dict[str, Any],
        existing_exit_orders: List[Dict[str, Any]],
    ) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        grid_cfg = self._moving_grid_config()
        position_side = self._extract_position_side(position)
        position_qty = self._extract_position_amount(position)
        meta: Dict[str, Any] = {
            "position_side": position_side,
            "position_qty": position_qty,
        }
        if position_side not in {"LONG", "SHORT"} or position_qty <= 0 or current_price <= 0:
            meta["blocked_reason"] = "no_position"
            return [], meta

        levels, runtime_meta = self._build_moving_grid_levels(
            symbol=symbol,
            current_price=current_price,
            direction_mode=position_side,
            market_data=market_data,
            existing_reference_orders=existing_exit_orders,
        )
        meta.update(runtime_meta)

        entry_price = self._to_float((position or {}).get("entry_price"), current_price)
        min_tp_ratio = max(0.0, self._normalize_percent_to_ratio(grid_cfg.get("position_min_tp_pct", 0.002), 0.002))
        exit_order_count = max(1, int(grid_cfg.get("position_exit_order_count", grid_cfg.get("active_order_count", 6))))
        if position_side == "LONG":
            trigger_price = max(current_price, entry_price * (1.0 + min_tp_ratio))
            side = "SELL"
            candidates = sorted(
                [lvl for lvl in levels if self._to_float(getattr(lvl, "price", 0.0), 0.0) > trigger_price],
                key=lambda lvl: self._to_float(getattr(lvl, "price", 0.0), 0.0),
            )
        else:
            trigger_price = min(current_price, entry_price * (1.0 - min_tp_ratio))
            side = "BUY"
            candidates = sorted(
                [lvl for lvl in levels if self._to_float(getattr(lvl, "price", 0.0), 0.0) < trigger_price],
                key=lambda lvl: self._to_float(getattr(lvl, "price", 0.0), 0.0),
        ...
        meta["exit_trigger_price"] = trigger_price
        candidates = candidates[:exit_order_count]
        if not candidates:
            meta["blocked_reason"] = "no_exit_candidates"
            return [], meta

        remaining_qty = position_qty
        targets: List[Dict[str, Any]] = []
        compressed_levels = 0
        for idx, lvl in enumerate(candidates):
            level_price = self._to_float(getattr(lvl, "price", 0.0), 0.0)
            if level_price <= 0 or remaining_qty <= 0:
                continue
            slots_left = max(1, len(candidates) - idx)
            raw_qty = remaining_qty / slots_left
            try:
                min_qty = float(self.client.ensure_min_notional_quantity(symbol, raw_qty, level_price))
                if min_qty > remaining_qty + 1e-12:
                    if targets:
                        compressed_levels += slots_left
                        break
                    qty = float(self.client.format_quantity(symbol, remaining_qty))
                else:
                    qty = float(self.client.format_quantity(symbol, min_qty))
            except Exception:
                qty = raw_qty
            if qty <= 0:
                continue
            price = self.fund_flow_execution_router._format_price(symbol, level_price)
            targets.append({"side": side, "price": price, "quantity": qty, "reduce_only": True})
            remaining_qty = max(0.0, remaining_qty - qty)

        if not targets:
            meta["blocked_reason"] = "exit_qty_rounded_to_zero"
            return [], meta
        meta["remaining_qty_after_targets"] = remaining_qty
        meta["compressed_exit_levels"] = compressed_levels
        return targets, meta

    def _sync_moving_grid_orders(
        self,
        *,
        symbol: str,
        current_price: float,
        ...
        grid_cfg = self._moving_grid_config()
        existing_entry_orders = self._list_open_entry_orders(symbol)
        existing_exit_orders = self._list_open_grid_exit_orders(symbol)
        if not grid_cfg.get("enabled", True):
            for order in [*existing_entry_orders, *existing_exit_orders]:
        ...
        position_side = self._extract_position_side(position)
        # 持仓补仓梯子不应被“新开仓窗口”强制打成中性，否则每次持仓复核都会撤掉后续补仓单。
        bias_allow_entry_window = allow_new_entries or position_side in {"LONG", "SHORT"}
        bias = self._evaluate_moving_grid_bias(symbol, market_data, bias_allow_entry_window)
        bias_direction_mode = self._moving_grid_direction_mode(bias)
        bias["entry_window_blocked"] = not bool(allow_new_entries)
        bias["replenish_window_override"] = bool(position_side in {"LONG", "SHORT"} and not allow_new_entries)
        entry_targets: List[Dict[str, Any]] = []
        exit_targets: List[Dict[str, Any]] = []
        entry_target_meta: Dict[str, Any] = {"direction_mode": bias_direction_mode}
        exit_target_meta: Dict[str, Any] = {"position_side": position_side}
        unified_risk_result = self._evaluate_unified_symbol_risk(
            symbol=symbol,
            current_price=current_price,
            account_summary=account_summary,
            position=position,
            positions_payload=self._position_snapshot_by_symbol(ConfigLoader.get_trading_symbols(self.config)),
            volatility_blocked=False,
            direction_mode=bias_direction_mode,
        )
        entry_blocked_by_unified_risk = self._should_block_entries_from_unified_risk(unified_risk_result)
        if entry_blocked_by_unified_risk:
            entry_target_meta["blocked_reason"] = (
                f"unified_risk:{unified_risk_result.action.value}:{unified_risk_result.reason}"
            )
            if (
                isinstance(unified_risk_result, RiskCheckResult)
                and unified_risk_result.action in {
                    UnifiedRiskAction.CANCEL_NON_CRITICAL_ORDERS,
                    UnifiedRiskAction.PAUSE_STRATEGY,
                    UnifiedRiskAction.STOP_STRATEGY,
                    UnifiedRiskAction.FORCE_CLOSE,
                ...
                for order in existing_entry_orders:
                    order_id = order.get("orderId")
                    if order_id is None:
                        continue
                    try:
                        ...
                        print(f"⚠️ {symbol} 统一风控撤销网格挂单失败: orderId={order_id} err={e}")
                existing_entry_orders = self._list_open_entry_orders(symbol)
        risk_guard_allows_open = (not risk_guard_enabled) or (not self._is_cooldown_active())
        can_manage_position_grid = (
            risk_guard_allows_open
            and (not block_new_entries_due_to_protection_gap)
            and (not entry_blocked_by_unified_risk)
        )
        can_open_new_position = allow_new_entries and can_manage_position_grid

        if position_side in {"LONG", "SHORT"}:
            exit_targets, exit_target_meta = self._compute_position_exit_grid_targets(
                symbol=symbol,
                current_price=current_price,
                position=position,
                market_data=market_data,
                existing_exit_orders=existing_exit_orders,
            )
            if bias_direction_mode == position_side and can_manage_position_grid:
                entry_targets, entry_target_meta = self._compute_moving_grid_targets(
                    symbol=symbol,
                    current_price=current_price,
                    direction_mode=position_side,
                    account_summary=account_summary,
                    position=position,
                    market_data=market_data,
                    existing_entry_orders=existing_entry_orders,
                )
            elif bias_direction_mode != position_side:
                entry_target_meta["blocked_reason"] = f"holding_{position_side.lower()}_bias={bias_direction_mode.lower()}"
            else:
                entry_target_meta["blocked_reason"] = "position_replenish_blocked"
        else:
            if can_open_new_position and bias_direction_mode in {"LONG", "SHORT"}:
                entry_targets, entry_target_meta = self._compute_moving_grid_targets(
                    symbol=symbol,
                    current_price=current_price,
                    direction_mode=bias_direction_mode,
                ...
                entry_target_meta["blocked_reason"] = "entry_window_or_risk_block"

        desired_entry_signatures = {
            self._grid_order_signature(item["side"], self._to_float(item.get("price"), 0.0))
            for item in entry_targets
        }
        desired_exit_signatures = {
            self._grid_order_signature(
                item["side"],
                self._to_float(item.get("price"), 0.0),
                self._to_float(item.get("quantity"), 0.0),
            )
            for item in exit_targets
        }
        existing_entry_signatures = set()
        existing_exit_signatures = set()
        cancelled_entry = 0
        cancelled_exit = 0
        for order in existing_entry_orders:
            side = str(order.get("side", "")).upper()
            price = self._to_float(order.get("price"), 0.0)
            signature = self._grid_order_signature(side, price)
            existing_entry_signatures.add(signature)
            if signature in desired_entry_signatures:
                continue
            order_id = order.get("orderId")
            if order_id is None:
                continue
            try:
                self.client.cancel_order(symbol, int(order_id))
                cancelled_entry += 1
                print(f"🧹 {symbol} 撤销旧网格挂单: side={side} price={price:.8f} orderId={order_id}")
            except Exception as e:
                print(f"⚠️ {symbol} 撤销旧网格挂单失败: orderId={order_id} err={e}")

        for order in existing_exit_orders:
            side = str(order.get("side", "")).upper()
            price = self._to_float(order.get("price"), 0.0)
            qty = self._to_float(order.get("origQty", order.get("quantity", 0.0)), 0.0)
            signature = self._grid_order_signature(side, price, qty)
            existing_exit_signatures.add(signature)
            if signature in desired_exit_signatures:
                continue
            order_id = order.get("orderId")
            if order_id is None:
                continue
            try:
                self.client.cancel_order(symbol, int(order_id))
                cancelled_exit += 1
                print(f"🧹 {symbol} 撤销退出网格挂单: side={side} price={price:.8f} qty={qty:.6f} orderId={order_id}")
            except Exception as e:
                print(f"⚠️ {symbol} 撤销退出网格挂单失败: orderId={order_id} err={e}")
        ...
        for item in entry_targets:
            ...
            signature = self._grid_order_signature(side, price)
            if signature in existing_entry_signatures or price <= 0 or qty <= 0:
                continue
            pre_order_result = self._check_unified_pre_order(
                symbol=symbol,
                order_request=self._build_unified_order_request(
                    symbol=symbol,
                    side=side,
                    price=price,
                    quantity=qty,
                ...
                current_price=current_price,
                positions_payload=self._position_snapshot_by_symbol(ConfigLoader.get_trading_symbols(self.config)),
                decision_side="LONG" if side == "BUY" else "SHORT",
            )
            if self._should_block_entries_from_unified_risk(pre_order_result):
                print(
                    f"⏭️ {symbol} 统一下单前风控阻止移动网格挂单: "
                    f"side={side} price={price:.8f} action={pre_order_result.action.value} "
                    f"risk={pre_order_result.risk_level.value} reason={pre_order_result.reason}"
                )
                continue
            params = {
                "symbol": symbol,
                ...
                "price": price,
                "quantity": qty,
                "newClientOrderId": self._make_moving_grid_client_order_id(
                    symbol=symbol,
                    side=side,
                    price=price,
                    kind="entry",
                ),
                "_allow_position_increase": True,
            }
            result = self.client._execute_order_v2(params=params, side=side, reduce_only=False)
            if isinstance(result, dict) and str(result.get("status", "")).lower() not in {"error", "failed"}:
                placed_entry += 1
                print(f"🧱 {symbol} 新增移动网格挂单: side={side} price={price:.8f} qty={qty}")
            else:
                print(f"⚠️ {symbol} 移动网格挂单失败: side={side} price={price:.8f} qty={qty} result={result}")
        ...
        for item in exit_targets:
            ...
            qty = self._to_float(item.get("quantity"), 0.0)
            signature = self._grid_order_signature(side, price, qty)
            if signature in existing_exit_signatures or price <= 0 or qty <= 0:
                continue
            params = {
                "symbol": symbol,
                "type": "LIMIT",
            ...
            try:
                result = self.client._execute_order_v2(params=params, side=side, reduce_only=True)
            except Exception as e:
                result = {"status": "error", "message": str(e)}
            if isinstance(result, dict) and str(result.get("status", "")).lower() not in {"error", "failed"}:
                placed_exit += 1
                print(f"🪜 {symbol} 新增退出网格挂单: side={side} price={price:.8f} qty={qty}")
            else:
                print(f"⚠️ {symbol} 退出网格挂单失败: side={side} price={price:.8f} qty={qty} result={result}")

        print(
            f"📐 {symbol} 移动网格同步: bias={bias.get('guide_direction')} "
            f"score={self._to_float(bias.get('guide_score'), 0.0):+.3f} "
            f"mode={bias_direction_mode} pos={position_side or 'FLAT'} "
            f"entry_targets={len(entry_targets)} exit_targets={len(exit_targets)} "
            f"placed_entry={placed_entry} placed_exit={placed_exit} "
            f"existing_entry={len(existing_entry_orders)} existing_exit={len(existing_exit_orders)} "
            f"range={self._to_float((entry_target_meta if entry_targets else exit_target_meta).get('range_pct'), 0.0):.4f} "
            f"anchor={self._to_float((entry_target_meta if entry_targets else exit_target_meta).get('anchor_price'), current_price):.8f}"
        )
        try:
            active_tf = str((bias.get("flow_context") or {}).get("active_timeframe") or market_data.get("trend_filter_timeframe") or "15m")
            kline = self._latest_kline_snapshot(symbol, active_tf)
            self._append_kline_debug_event(
                {
                    "cycle": int(self._cycle_sequence),
                    "mode": self.strategy_mode,
                    "event": "moving_grid_sync",
                    "symbol": symbol,
                    "action": "SYNC_GRID",
                    "status": "ok",
                    "reason": str(
                        entry_target_meta.get("blocked_reason")
                        or exit_target_meta.get("blocked_reason")
                        or ""
                    ),
                    "trigger_type": "moving_grid",
                    "direction": position_side or bias_direction_mode,
                    "guide_direction": str(bias.get("guide_direction", "BOTH")),
                    "guide_score": self._to_float(bias.get("guide_score"), 0.0),
                    "kline": kline,
                    "scores": {
                        "long_score": 0.0,
                        "short_score": 0.0,
                    ...
                    "market": {
                        "current_price": current_price,
                        "adx": self._to_float(((bias.get("raw_regime_info") or {}).get("adx")), 0.0),
                        "atr_pct": self._to_float(((bias.get("raw_regime_info") or {}).get("atr_pct")), 0.0),
                        "cvd": self._to_float((bias.get("flow_context") or {}).get("cvd_ratio"), 0.0),
                        "cvd_mom": self._to_float((bias.get("flow_context") or {}).get("cvd_momentum"), 0.0),
                        "oi_delta": self._to_float((bias.get("flow_context") or {}).get("oi_delta_ratio"), 0.0),
                        "funding": self._to_float((bias.get("flow_context") or {}).get("funding_rate"), 0.0),
                        "depth": self._to_float((bias.get("flow_context") or {}).get("depth_ratio"), 1.0),
                        "imbalance": self._to_float((bias.get("flow_context") or {}).get("imbalance"), 0.0),
                        "liq_norm": self._to_float((bias.get("flow_context") or {}).get("liquidity_delta_norm"), 0.0),
                    },
                    "risk": {
                        "target_portion": self._to_float(grid_cfg.get("budget_portion"), 0.0),
                        "current_portion": self._estimate_position_portion(position, account_summary),
                        "requested_leverage": int(grid_cfg.get("leverage", 1)),
                        "applied_leverage": int(grid_cfg.get("leverage", 1)),
                        "position_pre": self._position_snapshot_text(position),
                        "position_post": self._position_snapshot_text(position),
                        "open_order_count": len(self._list_open_entry_orders(symbol)) + len(self._list_open_grid_exit_orders(symbol)),
                        "entry_order_count": len(self._list_open_entry_orders(symbol)),
                        "exit_order_count": len(self._list_open_grid_exit_orders(symbol)),
                        "placed_orders": placed_entry + placed_exit,
                        "placed_entry_orders": placed_entry,
                        "placed_exit_orders": placed_exit,
                        "cancelled_orders": cancelled_entry + cancelled_exit,
                    ...
                    "debug": {
                        "bias": bias,
                        "entry_target_meta": entry_target_meta,
                        "exit_target_meta": exit_target_meta,
            ...
            print(f"⚠️ {symbol} 移动网格K线快照写入失败: {e}")
    ...
    def _has_pending_close_order(self, symbol: str) -> bool:
        ...
        for order in orders:
            ...
            if not (is_reduce or is_close):
                continue
            if self._is_managed_moving_grid_exit_order(order):
                continue
            order_type = str(order.get("type", "")).upper()
            strategy_type = str(order.get("strategyType", "")).upper()
            if "TAKE_PROFIT" in order_type or "STOP" in order_type:
    ...
    def _execute_and_log_decision(
        ...
        *,
        ...
        if decision.operation in (FundFlowOperation.BUY, FundFlowOperation.SELL):
            side = "BUY" if decision.operation == FundFlowOperation.BUY else "SELL"
            pre_order_result = self._check_unified_pre_order(
                symbol=symbol,
                order_request=self._build_unified_order_request(
                    symbol=symbol,
                    side=side,
                    price=current_price,
                    quantity=max(0.0, self._to_float(decision.target_portion_of_balance, 0.0)),
                    reduce_only=False,
                    source="fund_flow",
                ),
                account_summary=account_summary,
                ...
                current_price=current_price,
                positions_payload=self._position_snapshot_by_symbol(ConfigLoader.get_trading_symbols(self.config)),
                decision_side="LONG" if decision.operation == FundFlowOperation.BUY else "SHORT",
            )
            if self._should_block_entries_from_unified_risk(pre_order_result):
                print(
                    f"⏭️ {symbol} 统一下单前风控阻止执行: "
                    f"op={decision.operation.value.upper()} action={pre_order_result.action.value} "
                    f"risk={pre_order_result.risk_level.value} reason={pre_order_result.reason}"
                )
                return
        ...
        try:
            active_tf = str(md.get("active_timeframe") or flow_context.get("active_timeframe") or "15m")
            kline = self._latest_kline_snapshot(symbol, active_tf)
            ...
            self._append_kline_debug_event(
                {
                    ...
                    "symbol": symbol,
                    "action": decision.operation.value.upper(),
                    "status": display_status,
                    "reason": str(decision.reason or ""),
                    "trigger_type": trigger_type,
                    "direction": str(direction_lock or "-"),
                    "guide_direction": str(
                        combo_compare.get("guide_dir", combo_compare.get("active_dir", ev_direction))
                    ),
                    "guide_score": self._to_float(
                        combo_compare.get("guide_score", combo_compare.get("active_score", ev_score)),
                        0.0,
                    ),
                    "kline": kline,
                    "scores": {
                        ...
                        "score_15m_long": self._to_float(score_15m_md.get("long_score"), 0.0),
                        "score_15m_short": self._to_float(score_15m_md.get("short_score"), 0.0),
                        "score_5m_long": self._to_float(score_5m_md.get("long_score"), 0.0),
                        "score_5m_short": self._to_float(score_5m_md.get("short_score"), 0.0),
                        "final_long": self._to_float(final_score_md.get("long_score"), 0.0),
                        "final_short": self._to_float(final_score_md.get("short_score"), 0.0),
                    },
                    "market": {
                        "current_price": current_price,
                        "adx": regime_adx,
                        "atr_pct": regime_atr_pct,
                        "cvd": self._to_float(flow_context.get("cvd_ratio"), 0.0),
                        "cvd_mom": self._to_float(flow_context.get("cvd_momentum"), 0.0),
                        "oi_delta": self._to_float(flow_context.get("oi_delta_ratio"), 0.0),
                        "funding": self._to_float(flow_context.get("funding_rate"), 0.0),
                        "depth": self._to_float(flow_context.get("depth_ratio"), 1.0),
                        "imbalance": self._to_float(flow_context.get("imbalance"), 0.0),
                        "liq_norm": self._to_float(flow_context.get("liquidity_delta_norm"), 0.0),
                    },
                    "risk": {
                        "target_portion": self._to_float(decision.target_portion_of_balance, 0.0),
                        "current_portion": current_portion,
                        "requested_leverage": lev_req,
                        "applied_leverage": lev_applied,
                        "position_pre": self._position_snapshot_text(position),
                        "position_post": self._position_snapshot_text(position_for_log),
                        "open_order_count": len(self._list_open_entry_orders(symbol)),
                        "placed_orders": 0,
                        "cancelled_orders": 0,
                    },
                    "params_snapshot": {
                        "strategy_mode": self.strategy_mode,
                        "open_threshold": self._to_float(ff_cfg.get("open_threshold"), 0.0),
                        "long_open_threshold": self._to_float(ff_cfg.get("long_open_threshold"), 0.0),
                        "short_open_threshold": self._to_float(ff_cfg.get("short_open_threshold"), 0.0),
                        "close_threshold": self._to_float(ff_cfg.get("close_threshold"), 0.0),
                        "stop_loss_pct": self._to_float(ff_cfg.get("stop_loss_pct", risk_cfg.get("stop_loss_default_percent", 0.0)), 0.0),
                        "take_profit_pct": self._to_float(ff_cfg.get("take_profit_pct", risk_cfg.get("take_profit_default_percent", 0.0)), 0.0),
                        "direction_guide": self.fund_flow_decision_engine.get_direction_guide_snapshot(),
                    },
                    "debug": {
    ...
    def run_cycle(self, allow_new_entries: bool = True, ai_review_mode: str = "disabled") -> None:
        if not self._ensure_startup_risk_check():
            return
    ...
    def _run_cycle_impl(self, allow_new_entries: bool = True, ai_review_mode: str = "disabled") -> None:
        ...
        if context is None:
            return
        if self._moving_grid_enabled:
            self._run_moving_grid_cycle(context)
            return
    ...
    def _run_moving_grid_cycle(self, context: Dict[str, Any]) -> None:
        symbols_raw = context.get("symbols")
        symbols = symbols_raw if isinstance(symbols_raw, list) else []
        cycle_start_ts = self._to_float(context.get("cycle_start_ts"), time.time())
        max_cycle_runtime_seconds = max(0.0, self._to_float(context.get("max_cycle_runtime_seconds"), 0.0))
        ...
        for idx, symbol in enumerate(symbols):
            if max_cycle_runtime_seconds > 0:
                ...
                if elapsed_before >= max_cycle_runtime_seconds:
                    ...
                    break
            self._process_symbol_moving_grid(symbol=symbol, idx=idx, context=context)

    def _process_symbol_moving_grid(self, symbol: str, idx: int, context: Dict[str, Any]) -> None:
        symbols_raw = context.get("symbols")
        symbols = symbols_raw if isinstance(symbols_raw, list) else []
        ...
        sla_cfg = context.get("sla_cfg") if isinstance(context.get("sla_cfg"), dict) else {}
        ...
        protection_gap_symbols = context.get("protection_gap_symbols")
        if not isinstance(protection_gap_symbols, list):
        ...
        try:
            ...
            print(f"❌ {symbol} 移动网格处理异常: {e}")
        finally:
            context["block_new_entries_due_to_protection_gap"] = block_new_entries_due_to_protection_gap
    ...
    def _prepare_cycle_context(
        ...
        return {
            ...
            "cycle_id": int(self._cycle_sequence),
    ...
    def _execute_symbol_signal_decision(
        ...
        for _ in (0,):
            ...
            unified_risk_result = self._evaluate_unified_symbol_risk(
                ...
                decision=decision,
                positions_payload=self._position_snapshot_by_symbol(ConfigLoader.get_trading_symbols(self.config)),
                volatility_blocked=bool(volatility_guard.get("blocked")),
            )
            if self._should_block_entries_from_unified_risk(unified_risk_result) and decision.operation in (
                FundFlowOperation.BUY,
                FundFlowOperation.SELL,
            ):
                block_reason = (
                    f"UNIFIED_RISK_BLOCK action={unified_risk_result.action.value} "
                    f"risk={unified_risk_result.risk_level.value} reason={unified_risk_result.reason}"
                )
                base_reason = str(decision.reason or "").strip()
                print(f"⏭️ {symbol} 统一风控阻止开仓信号: {block_reason}")
                decision = FundFlowDecision(
                    operation=FundFlowOperation.HOLD,
                    symbol=symbol,
                    target_portion_of_balance=0.0,
        ...
        while True:
            ...
            if has_position:
                ...
                if position_review_due and entry_review_due:
                    print(
                        f"\n=== {self._cycle_mode_label()} cycle {cycles + 1} @ {now_utc} UTC === "
