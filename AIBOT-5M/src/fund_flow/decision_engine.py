...
class FundFlowDecisionEngine:
    ...
    def _trend_capture_config(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "tf_exec": str(conf.get("tf_exec", "15m")),
            ...
            "risk_guard_enabled": bool(conf.get("risk_guard_enabled", True)),
            "risk_guard_stop_loss_mult": self._to_float(conf.get("risk_guard_stop_loss_mult"), 1.5),
            "risk_guard_stop_loss_atr_mult": self._to_float(conf.get("risk_guard_stop_loss_atr_mult"), 1.25),
            "risk_guard_stop_loss_max_pct": self._normalize_pct_ratio(conf.get("risk_guard_stop_loss_max_pct"), 0.009),
            ...
            "entry_high_score_exempt_long": self._to_float(
                tc.get("entry_high_score_exempt_long"),
                max(
                    self._to_float(ff.get("long_open_threshold"), self.long_open_threshold) + 0.05,
                    0.14,
                ),
            ),
            "entry_high_score_exempt_short": self._to_float(
                tc.get("entry_high_score_exempt_short"),
                max(
                    self._to_float(ff.get("short_open_threshold"), self.short_open_threshold) + 0.05,
                    0.14,
                ),
            ),
    ...
    def _resolve_entry_stop_loss_pct(
        ...
        *,
        ...
        if not bool(cfg.get("dynamic_stop_loss_enabled", False)):
            ...
            if resolved <= 0:
                resolved = max_ratio
            resolved = min(max_ratio, max(min_ratio, resolved))
            if not bool(cfg.get("risk_guard_enabled", True)):
                return resolved
            side_up = str(entry_side or "").upper()
            if side_up not in {"LONG", "SHORT"}:
                return resolved
            guard = self._evaluate_mtf_macd_stop_loss_guard(side_up, market_flow_context)
            if not bool(guard.get("active", False)):
                return resolved
            widened = resolved
            widened = max(widened, base_ratio * max(1.0, self._to_float(cfg.get("risk_guard_stop_loss_mult"), 1.5)))
            if atr_pct > 0:
                widened = max(
                    widened,
                    atr_pct * max(0.1, self._to_float(cfg.get("risk_guard_stop_loss_atr_mult"), 1.25)),
                )
            cap_ratio = max(0.0, self._normalize_pct_ratio(cfg.get("risk_guard_stop_loss_max_pct"), 0.009))
            if cap_ratio > 0:
                widened = min(cap_ratio, widened)
            return max(resolved, widened)
        return resolved

    @staticmethod
    def _macd_side_supports_entry(
        side: str,
        *,
        cross: str,
        hist: float,
        ...
        if side_up == "LONG":
            return bool(cross_up == "GOLDEN" or (hist > 0 and (hist_expand_up or hist_delta >= 0.0)))
        if side_up == "SHORT":
            return bool(cross_up == "DEAD" or (hist < 0 and (hist_expand_down or hist_delta <= 0.0)))
        return False

    @staticmethod
    def _macd_anchor_strongly_opposes_entry(
        side: str,
        *,
        cross: str,
        hist: float,
    ...
    def _evaluate_mtf_macd_stop_loss_guard(
        self,
        entry_side: str,
        market_flow_context: Dict[str, Any],
    ) -> Dict[str, Any]:
        snap = market_flow_context.get("_ma10_macd_confluence") if isinstance(market_flow_context, dict) else {}
        snap = snap if isinstance(snap, dict) else {}
        side_up = str(entry_side).upper()
        if side_up not in {"LONG", "SHORT"}:
            return {"active": False}

        exec_cross = str(snap.get("macd_15m_cross", snap.get("macd_exec_cross", snap.get("macd_5m_cross", "NONE")))).upper()
        exec_hist = self._to_float(snap.get("macd_15m_hist", snap.get("macd_5m_hist", snap.get("macd_hist", 0.0))), 0.0)
        exec_hist_delta = self._to_float(
            snap.get("macd_15m_hist_delta", snap.get("macd_5m_hist_delta", snap.get("macd_hist_delta", 0.0))),
            0.0,
        )
        exec_expand_up = bool(snap.get("macd_15m_hist_expand_up", snap.get("macd_5m_hist_expand_up", False)))
        exec_expand_down = bool(snap.get("macd_15m_hist_expand_down", snap.get("macd_5m_hist_expand_down", False)))
        anchor_cross = str(snap.get("macd_1h_cross", "NONE")).upper()
        anchor_hist = self._to_float(snap.get("macd_1h_hist", 0.0), 0.0)
        anchor_hist_delta = self._to_float(snap.get("macd_1h_hist_delta", 0.0), 0.0)
        anchor_expand_up = bool(snap.get("macd_1h_hist_expand_up", False))
        anchor_expand_down = bool(snap.get("macd_1h_hist_expand_down", False))

        exec_support = self._macd_side_supports_entry(
            side_up,
            cross=exec_cross,
            hist=exec_hist,
        ...
        anchor_opposite_strong = self._macd_anchor_strongly_opposes_entry(
            side_up,
            cross=anchor_cross,
            hist=anchor_hist,
        ...
        return {
            ...
            "exec_support": bool(exec_support),
            "anchor_opposite_strong": bool(anchor_opposite_strong),
        }
    ...
    def _compute_entry_confluence_v2(
        ...
        tf5 = timeframes.get(str(cfg.get("tf_exec", "15m"))) if isinstance(timeframes, dict) else {}
    ...
    def _resolve_entry_mode(
        ...
        entry_high_score_exempt_long = self._to_float(
            cfg.get("entry_high_score_exempt_long"),
            max(open_long_th + 0.05, 0.14),
        )
        entry_high_score_exempt_short = self._to_float(
        ...
        regime_adx = self._to_float(regime_info.get("adx"), self._to_float(regime_info.get("regime_adx"), 0.0))
        operation = Operation.HOLD
        side = "NONE"
        entry_mode = "HOLD"
        entry_stage = 0
        ...
        if operation in (Operation.BUY, Operation.SELL):
            ...
            if regime == "TREND":
                ...
                if (
                    require_adx_slope
                    and adx_slope_15m <= required_adx_slope
                    and final_long < entry_high_score_exempt_long
                    and final_short < entry_high_score_exempt_short
                    and regime_adx < adx_strong_trend_exempt
                ):
    ...
    def decide(
        ...
        metadata_base["open_thresholds"] = {"long": long_threshold, "short": short_threshold}
        ...
        stop_loss_cfg = {**trend_cfg, **engine_params}
        stop_loss_pct_long = self._resolve_entry_stop_loss_pct(
        ...
        stop_loss_pct_short = self._resolve_entry_stop_loss_pct(
            base_stop_loss_pct=engine_params.get("stop_loss_pct"),
            regime_info=regime_info,
            market_flow_context=market_flow_context,
            cfg=stop_loss_cfg,
        ...
        sl_enabled = max(stop_loss_pct_long, stop_loss_pct_short) > 0
        tp_long_price = price * (1.0 + take_profit_pct) if tp_enabled else None
        sl_long_price = price * (1.0 - stop_loss_pct_long) if stop_loss_pct_long > 0 else None
        tp_short_price = price * (1.0 - take_profit_pct) if tp_enabled else None
        sl_short_price = price * (1.0 + stop_loss_pct_short) if stop_loss_pct_short > 0 else None
        metadata_base["entry_dynamic_stop_loss_pct"] = max(stop_loss_pct_long, stop_loss_pct_short)
        metadata_base["entry_dynamic_stop_loss_pct_long"] = stop_loss_pct_long
        metadata_base["entry_dynamic_stop_loss_pct_short"] = stop_loss_pct_short
                    ...
                    FundFlowDecision(
                        ...
                        metadata={
                            ...
                            **range_veto,
                            ...
                            "tp_sl": {"tp_pct": take_profit_pct, "sl_pct": stop_loss_pct_long, "tp_enabled": tp_enabled, "sl_enabled": stop_loss_pct_long > 0},
                            "entry_mode": entry_mode,
                            "entry_stage": entry_stage,
                    ...
                    FundFlowDecision(
                        ...
                        metadata={
                            ...
                            **range_veto,
                            "open_thresholds": {"long": long_threshold, "short": short_threshold},
                            "leverage_model": {"min": min_lev, "max": max_lev, "levels": allowed_levs, "picked": leverage},
                            "tp_sl": {"tp_pct": take_profit_pct, "sl_pct": stop_loss_pct_short, "tp_enabled": tp_enabled, "sl_enabled": stop_loss_pct_short > 0},
                            "entry_mode": entry_mode,
                            "entry_stage": entry_stage,
                            "entry_size_mult": entry_size_mult,
                            "tp_levels": self._build_tp_levels_metadata(
