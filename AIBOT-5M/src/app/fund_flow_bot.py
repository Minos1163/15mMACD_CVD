...
class TradingBot:
    ...
    def _execution_timeframe(self) -> str:
        ff_cfg = self.config.get("fund_flow", {}) or {}
        conf_raw = ff_cfg.get("ma10_macd_confluence", {})
        conf = conf_raw if isinstance(conf_raw, dict) else {}
        tf = conf.get("tf_exec") or conf.get("exec_tf") or ff_cfg.get("decision_timeframe") or ff_cfg.get("signal_timeframe") or "15m"
        tf_norm = str(tf).strip().lower()
        allowed = {"1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h"}
        return tf_norm if tf_norm in allowed else "15m"
    ...
    def _ai_review_config(self) -> Dict[str, Any]:
        ...
        position_tf_seconds = self._parse_timeframe_seconds(ai_cfg.get("position_timeframe")) or decision_tf_seconds or 900
    ...
    def _ai_entry_guard(
        *,
        ...
        macd_cross = str(
            ctx.get("macd_cross_exec", ctx.get("macd_cross_15m", ctx.get("macd_cross_5m", "NONE")))
        ).upper()
        macd_zone = str(
            ctx.get("macd_zone_exec", ctx.get("macd_zone_15m", ctx.get("macd_zone_5m", "NEAR_ZERO")))
        ).upper()
    ...
    def _ma10_macd_confluence_config(self) -> Dict[str, Any]:
        ...
        tf_exec = str(raw.get("tf_exec", raw.get("exec_tf", self.config.get("fund_flow", {}).get("decision_timeframe", "15m"))) or "15m").strip().lower()
        ...
        legacy_fast = max(2, int(self._to_float(raw.get("macd_fast", 12), 12)))
        legacy_slow = max(3, int(self._to_float(raw.get("macd_slow", 26), 26)))
        legacy_signal = max(2, int(self._to_float(raw.get("macd_signal", 9), 9)))
        macd_timeframes_raw = raw.get("macd_timeframes", {}) if isinstance(raw.get("macd_timeframes"), dict) else {}

        def _normalize_macd_params(raw_params: Any, fallback: Dict[str, int]) -> Dict[str, int]:
            params = raw_params if isinstance(raw_params, dict) else {}
            fast = max(2, int(self._to_float(params.get("fast", fallback["fast"]), fallback["fast"])))
            slow = max(3, int(self._to_float(params.get("slow", fallback["slow"]), fallback["slow"])))
            signal = max(2, int(self._to_float(params.get("signal", fallback["signal"]), fallback["signal"])))
            if slow <= fast:
                slow = fast + 1
            return {
                "fast": fast,
        ...
        macd_timeframes: Dict[str, Dict[str, int]] = {}
        for timeframe, params in macd_timeframes_raw.items():
            tf_key = str(timeframe or "").strip().lower()
            if tf_key not in allowed_tf:
                continue
            macd_timeframes[tf_key] = _normalize_macd_params(
                params,
                fallback={"fast": legacy_fast, "slow": legacy_slow, "signal": legacy_signal},
            )

        exec_params = _normalize_macd_params(
            raw.get("macd_params_exec", macd_timeframes.get(tf_exec)),
            fallback=macd_timeframes.get(tf_exec, {"fast": legacy_fast, "slow": legacy_slow, "signal": legacy_signal}),
        )
        anchor_params = _normalize_macd_params(
            raw.get("macd_params_anchor", macd_timeframes.get(tf_anchor)),
            fallback=macd_timeframes.get(tf_anchor, {"fast": legacy_fast, "slow": legacy_slow, "signal": legacy_signal}),
        )
        ...
        return {
            ...
            "macd_fast": int(exec_params["fast"]),
            "macd_slow": int(exec_params["slow"]),
            "macd_signal": int(exec_params["signal"]),
            "macd_exec_fast": int(exec_params["fast"]),
            "macd_exec_slow": int(exec_params["slow"]),
            "macd_exec_signal": int(exec_params["signal"]),
            "macd_anchor_fast": int(anchor_params["fast"]),
            "macd_anchor_slow": int(anchor_params["slow"]),
            "macd_anchor_signal": int(anchor_params["signal"]),
            ...
            "risk_guard_stop_loss_mult": min(
                3.0,
                max(1.0, self._to_float(raw.get("risk_guard_stop_loss_mult", 1.5), 1.5)),
            ),
            "risk_guard_stop_loss_atr_mult": min(
                3.0,
                max(0.5, self._to_float(raw.get("risk_guard_stop_loss_atr_mult", 1.25), 1.25)),
            ),
            "risk_guard_stop_loss_max_pct": min(
                0.05,
                max(0.0, self._normalize_percent_to_ratio(raw.get("risk_guard_stop_loss_max_pct", 0.009), 0.009)),
            ),
            "risk_guard_skip_conflict_tighten": bool(raw.get("risk_guard_skip_conflict_tighten", True)),
            "risk_guard_skip_conflict_reduce": bool(raw.get("risk_guard_skip_conflict_reduce", True)),
        }
    ...
    def _storage_sync_config(self) -> Dict[str, Any]:
        ff_cfg = self.config.get("fund_flow", {}) or {}
        raw = ff_cfg.get("storage_sync", {}) if isinstance(ff_cfg.get("storage_sync"), dict) else {}
        periods_raw = raw.get("kline_periods", ["1m", "5m"])
        periods: List[str] = []
        if isinstance(periods_raw, list):
            for item in periods_raw:
                tf = str(item or "").strip().lower()
                if tf in {"1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h"} and tf not in periods:
                    periods.append(tf)
        if not periods:
            periods = ["1m", "5m"]
        return {
            "enabled": bool(raw.get("enabled", True)),
            "persist_confluence": bool(raw.get("persist_confluence", True)),
            "market": str(raw.get("market", "futures") or "futures").strip().lower(),
            "environment": str(raw.get("environment", "live") or "live").strip().lower(),
            "kline_periods": periods,
            "kline_limit_per_cycle": max(2, int(self._to_float(raw.get("kline_limit_per_cycle", 12), 12))),
        }


    def _compute_ma10_macd_confluence(self, symbol: str, cfg: Dict[str, Any]) -> Dict[str, Any]:
        tf_exec = str(cfg.get("tf_exec", self._execution_timeframe()) or self._execution_timeframe())
        ...
        macd_exec_fast = int(cfg.get("macd_exec_fast", cfg.get("macd_fast", 12)))
        macd_exec_slow = int(cfg.get("macd_exec_slow", cfg.get("macd_slow", 26)))
        macd_exec_signal = int(cfg.get("macd_exec_signal", cfg.get("macd_signal", 9)))
        if macd_exec_slow <= macd_exec_fast:
            macd_exec_slow = macd_exec_fast + 1
        macd_anchor_fast = int(cfg.get("macd_anchor_fast", cfg.get("macd_fast", 12)))
        macd_anchor_slow = int(cfg.get("macd_anchor_slow", cfg.get("macd_slow", 26)))
        macd_anchor_signal = int(cfg.get("macd_anchor_signal", cfg.get("macd_signal", 9)))
        if macd_anchor_slow <= macd_anchor_fast:
            macd_anchor_slow = macd_anchor_fast + 1
        ...
        ma10_exec = self._sma(closes_exec, ma_period)
        ...
        return {
            "ma10_exec": float(ma10_exec),
            "ma10_15m": float(ma10_exec),
            "ma10_5m": float(ma10_exec),
            "ma10_1h": float(ma10_1h),
            "ma10_1h_slope": float(slope),
            "ma10_1h_bias": int(bias),
            "last_open_exec": float(last_open_exec),
            "last_open_15m": float(last_open_exec),
            "last_open_5m": float(last_open_exec),
            "last_close_exec": float(last_close_exec),
            "last_close_15m": float(last_close_exec),
            "last_close_5m": float(last_close_exec),
            "last_close_1h": float(last_close_anchor),
            "macd_5m": float(macd_state.get("macd", 0.0)),
            "macd_15m": float(macd_state.get("macd", 0.0)),
            "macd_5m_signal": float(macd_state.get("signal", 0.0)),
            "macd_15m_signal": float(macd_state.get("signal", 0.0)),
            "macd_5m_hist": float(macd_state.get("hist", 0.0)),
            "macd_15m_hist": float(macd_state.get("hist", 0.0)),
            ...
            "macd_exec_zone": str(macd_state.get("zone", "NEAR_ZERO")),
            "macd_15m_zone": str(macd_state.get("zone", "NEAR_ZERO")),
            "macd_5m_zone": str(macd_state.get("zone", "NEAR_ZERO")),
            "macd_exec_hist_expand": bool(macd_state.get("hist_expand", False)),
            "macd_15m_hist_expand": bool(macd_state.get("hist_expand", False)),
            "macd_5m_hist_expand": bool(macd_state.get("hist_expand", False)),
            "macd_15m_hist_expand_up": bool(macd_state.get("hist_expand_up", False)),
    ...
    def _inject_confluence_into_flow_context(
        ...
        tf_exec = str(cfg.get("tf_exec", self._execution_timeframe()) or self._execution_timeframe()).strip().lower()
        tf_ctx_raw = timeframes.get(tf_exec)
        tf_ctx: Dict[str, Any] = tf_ctx_raw if isinstance(tf_ctx_raw, dict) else {}
        tf_ctx.update(
            {
                "last_open": self._to_float(
                    confluence.get("last_open_exec", confluence.get("last_open_5m")),
                    self._to_float(tf_ctx.get("last_open"), 0.0),
                ),
                "last_close": self._to_float(
                    confluence.get("last_close_exec", confluence.get("last_close_5m")),
                    self._to_float(tf_ctx.get("last_close"), 0.0),
                ),
        ...
        tf_anchor = str(cfg.get("tf_anchor", "1h") or "1h").strip().lower()
        tf_anchor_raw = timeframes.get(tf_anchor)
        tf_anchor_ctx: Dict[str, Any] = tf_anchor_raw if isinstance(tf_anchor_raw, dict) else {}
        tf_anchor_ctx.update(
            {
                "last_close": self._to_float(
                    confluence.get("last_close_1h"),
                    self._to_float(tf_anchor_ctx.get("last_close"), 0.0),
                ),
                "macd": self._to_float(
                    confluence.get("macd_1h"),
                    self._to_float(tf_anchor_ctx.get("macd"), 0.0),
                ),
                "signal": self._to_float(
                    confluence.get("macd_1h_signal"),
                    self._to_float(tf_anchor_ctx.get("signal"), 0.0),
                ),
                "macd_hist": self._to_float(
                    confluence.get("macd_1h_hist"),
                    self._to_float(tf_anchor_ctx.get("macd_hist"), 0.0),
                ),
                "macd_hist_norm": self._to_float(
                    confluence.get("macd_1h_hist_norm"),
                    self._to_float(tf_anchor_ctx.get("macd_hist_norm"), 0.0),
                ),
                ...
                "macd_hist_expand": bool(
                    confluence.get("macd_1h_hist_expand", tf_anchor_ctx.get("macd_hist_expand", False))
                ),
                "macd_hist_expand_up": bool(
                    confluence.get("macd_1h_hist_expand_up", tf_anchor_ctx.get("macd_hist_expand_up", False))
                ),
                ...
                "ma10": self._to_float(
                    ...
                    self._to_float(tf_anchor_ctx.get("ma10"), 0.0),
                ),
                "ma10_bias": int(self._to_int(confluence.get("ma10_1h_bias"), self._to_int(tf_anchor_ctx.get("ma10_bias"), 0))),
            }
        )
        timeframes[tf_anchor] = tf_anchor_ctx
        flow_context["timeframes"] = timeframes
        flow_context["_ma10_macd_confluence"] = dict(confluence)
        flow_context["macd_cross_exec"] = str(confluence.get("macd_cross", flow_context.get("macd_cross_exec", "NONE"))).upper()
        flow_context["macd_zone_exec"] = str(confluence.get("macd_zone", flow_context.get("macd_zone_exec", "NEAR_ZERO"))).upper()
        flow_context["macd_hist_expand_exec"] = bool(confluence.get("macd_exec_hist_expand", confluence.get("macd_5m_hist_expand", flow_context.get("macd_hist_expand_exec", False))))
        flow_context["kdj_cross_exec"] = str(confluence.get("kdj_cross", flow_context.get("kdj_cross_exec", "NONE"))).upper()
        flow_context["kdj_zone_exec"] = str(confluence.get("kdj_zone", flow_context.get("kdj_zone_exec", "MID"))).upper()
        if tf_exec == "15m":
            flow_context["macd_cross_15m"] = flow_context["macd_cross_exec"]
            flow_context["macd_zone_15m"] = flow_context["macd_zone_exec"]
            flow_context["macd_hist_expand_15m"] = flow_context["macd_hist_expand_exec"]
            flow_context["kdj_cross_15m"] = flow_context["kdj_cross_exec"]
            flow_context["kdj_zone_15m"] = flow_context["kdj_zone_exec"]
        flow_context["macd_cross_1h"] = str(confluence.get("macd_1h_cross", flow_context.get("macd_cross_1h", "NONE"))).upper()
        flow_context["macd_zone_1h"] = str(confluence.get("macd_1h_zone", flow_context.get("macd_zone_1h", "NEAR_ZERO"))).upper()
        flow_context["macd_hist_expand_1h"] = bool(
            confluence.get("macd_1h_hist_expand", flow_context.get("macd_hist_expand_1h", False))
        )
    ...
    @staticmethod
    def _macd_side_supports_position(
        side: str,
        *,
        cross: str,
        hist: float,
        hist_delta: float,
        hist_expand_up: bool,
        hist_expand_down: bool,
    ) -> bool:
        side_up = str(side).upper()
        cross_up = str(cross).upper()
        if side_up == "LONG":
            return bool(
                cross_up == "GOLDEN"
                or (hist > 0 and (hist_expand_up or hist_delta >= 0.0))
            )
        if side_up == "SHORT":
            return bool(
                cross_up == "DEAD"
                or (hist < 0 and (hist_expand_down or hist_delta <= 0.0))
            )
        return False

    @staticmethod
    def _macd_anchor_strongly_opposes_position(
        side: str,
        *,
        cross: str,
        hist: float,
    ...
    def _evaluate_macd_mtf_risk_guard(
        self,
        position_side: str,
        metadata: Dict[str, Any],
        cfg: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        conf_cfg = cfg or self._ma10_macd_confluence_config()
        side_up = str(position_side).upper()
        md = metadata if isinstance(metadata, dict) else {}
        if side_up not in {"LONG", "SHORT"} or not bool(conf_cfg.get("enabled", True)) or not bool(
            conf_cfg.get("risk_guard_enabled", True)
        ):
            return {"enabled": False, "active": False, "exec_support": False, "anchor_opposite_strong": False}

        exec_cross = str(md.get("macd_15m_cross", md.get("macd_exec_cross", md.get("macd_5m_cross", "NONE")))).upper()
        exec_hist = self._to_float(md.get("macd_15m_hist", md.get("macd_5m_hist", md.get("macd_hist", 0.0))), 0.0)
        exec_hist_delta = self._to_float(
            md.get("macd_15m_hist_delta", md.get("macd_5m_hist_delta", md.get("macd_hist_delta", 0.0))),
            0.0,
        )
        exec_expand_up = bool(md.get("macd_15m_hist_expand_up", md.get("macd_5m_hist_expand_up", False)))
        exec_expand_down = bool(md.get("macd_15m_hist_expand_down", md.get("macd_5m_hist_expand_down", False)))

        anchor_cross = str(md.get("macd_1h_cross", "NONE")).upper()
        anchor_hist = self._to_float(md.get("macd_1h_hist", 0.0), 0.0)
        anchor_hist_delta = self._to_float(md.get("macd_1h_hist_delta", 0.0), 0.0)
        anchor_expand_up = bool(md.get("macd_1h_hist_expand_up", False))
        anchor_expand_down = bool(md.get("macd_1h_hist_expand_down", False))

        exec_support = self._macd_side_supports_position(
            side_up,
            cross=exec_cross,
            hist=exec_hist,
            hist_delta=exec_hist_delta,
            hist_expand_up=exec_expand_up,
            hist_expand_down=exec_expand_down,
        )
        anchor_opposite_strong = self._macd_anchor_strongly_opposes_position(
            side_up,
            cross=anchor_cross,
            hist=anchor_hist,
            hist_delta=anchor_hist_delta,
            hist_expand_up=anchor_expand_up,
            hist_expand_down=anchor_expand_down,
        )
        active = bool(exec_support and not anchor_opposite_strong)
        return {
            "enabled": True,
            "active": active,
            "side": side_up,
            "exec_support": exec_support,
            "anchor_opposite_strong": anchor_opposite_strong,
            "exec_cross": exec_cross,
            "exec_hist": exec_hist,
            "exec_hist_delta": exec_hist_delta,
            "anchor_cross": anchor_cross,
            "anchor_hist": anchor_hist,
            "anchor_hist_delta": anchor_hist_delta,
        }
    ...
    def _build_dca_decision(
        ...
        *,
        ...
        bot_config = self.config if isinstance(getattr(self, "config", None), dict) else {}
        leverage = int(
            self._to_float(
                ...
                getattr(
                    ...
                    "default_leverage",
                    ConfigLoader.get_default_leverage(bot_config),
                ),
            )
        )
    ...
    def _startup_market_preload_config(self) -> Dict[str, Any]:
        ...
        kline_interval = str(startup_cfg.get("preload_market_kline_interval", "15m") or "15m").strip().lower()
    ...
    def _persist_runtime_klines(self, symbol: str, *, sync_cfg: Optional[Dict[str, Any]] = None) -> Dict[str, int]:
        cfg = sync_cfg if isinstance(sync_cfg, dict) else self._storage_sync_config()
        if not bool(cfg.get("enabled", True)) or self.fund_flow_storage is None:
            return {}
        market = str(cfg.get("market", "futures") or "futures").strip().lower()
        environment = str(cfg.get("environment", "live") or "live").strip().lower()
        limit = max(2, int(self._to_float(cfg.get("kline_limit_per_cycle", 12), 12)))
        out: Dict[str, int] = {}
        for period in cfg.get("kline_periods", []):
            tf = str(period or "").strip().lower()
            if not tf:
                continue
            try:
                rows_raw = self.client.get_klines(symbol, tf, limit=limit) or []
            except Exception as e:
                print(f"⚠️ {symbol} {tf} K线同步抓取失败: {e}")
                continue
            payload: List[Dict[str, Any]] = []
            for row in rows_raw:
                if not isinstance(row, list) or len(row) < 6:
                    continue
                ts = self._coerce_utc_datetime(row[0])
                if ts is None:
                    continue
                payload.append(
                    {
                        "timestamp": ts,
                        "open": self._to_float(row[1], 0.0),
                        "high": self._to_float(row[2], 0.0),
                        "low": self._to_float(row[3], 0.0),
                        "close": self._to_float(row[4], 0.0),
                        "volume": self._to_float(row[5], 0.0),
                    }
                )
            inserted = self._safe_storage_call(
                "upsert_klines",
                exchange="binance",
                symbol=symbol,
                market=market,
            ...
            out[tf] = int(inserted or 0)
        return out

    def _persist_ma10_macd_confluence_snapshot(
        self,
        symbol: str,
        *,
        ...
        storage_cfg = sync_cfg if isinstance(sync_cfg, dict) else self._storage_sync_config()
        if not bool(storage_cfg.get("enabled", True)) or not bool(storage_cfg.get("persist_confluence", True)):
            return
        if self.fund_flow_storage is None:
            return
        snap = dict(confluence or {})
        if not snap:
            return
        ts = timestamp if isinstance(timestamp, datetime) else datetime.now(timezone.utc)
        conf_cfg = cfg if isinstance(cfg, dict) else self._ma10_macd_confluence_config()
        self._safe_storage_call(
            "upsert_ma10_macd_confluence_snapshot",
            exchange="binance",
            symbol=symbol,
            timestamp=ts,
            exec_timeframe=str(conf_cfg.get("tf_exec", "15m") or "15m"),
            anchor_timeframe=str(conf_cfg.get("tf_anchor", "1h") or "1h"),
            snapshot=snap,
        )
    ...
    def _execute_and_log_decision(
        ...
        *,
        ...
        if decision.operation == FundFlowOperation.HOLD:
            ...
            decision_source = str(md.get("decision_source", "")).strip()
            entry_hard_filters_raw = md.get("entry_hard_filters")
            entry_hard_filters = []
            if isinstance(entry_hard_filters_raw, list):
                entry_hard_filters = [
                    str(item).strip()
                    for item in entry_hard_filters_raw
                    if str(item).strip()
                ]
            ...
            long_pass = final_long_score >= long_threshold > 0.0
            short_pass = final_short_score >= short_threshold > 0.0
            ...
            if decision_source == "entry_hard_filter_blocked":
                inferred_side = "LONG" if final_long_score >= final_short_score else "SHORT"
                if any("long" in name for name in entry_hard_filters) and not any(
                    "short" in name for name in entry_hard_filters
                ):
                    inferred_side = "LONG"
                elif any("short" in name for name in entry_hard_filters) and not any(
                    "long" in name for name in entry_hard_filters
                ):
                    inferred_side = "SHORT"
                print(
                    "   HOLD归因: "
                    f"entry_hard_filter_blocked={",".join(entry_hard_filters) or "-"}, "
                    f"side={inferred_side}, "
                    f"long={final_long_score:.3f}/{long_threshold:.3f}, "
                    f"short={final_short_score:.3f}/{short_threshold:.3f}, "
                    f"lock={direction_lock or '-'}"
                )
            elif direction_lock_applied and (blocked_long or blocked_short):
                ...
                print(
                    "   HOLD归因: "
                    f"hold_source={decision_source or 'none'}, "
                    f"long={final_long_score:.3f}/{long_threshold:.3f}, "
                    f"short={final_short_score:.3f}/{short_threshold:.3f}, "
                    f"lock={direction_lock or '-'}"
                )
    ...
    def _execute_symbol_signal_decision(
        ...
        for _ in (0,):
            ...
            storage_sync_cfg = self._storage_sync_config()
            if bool(storage_sync_cfg.get("enabled", True)):
                self._persist_runtime_klines(symbol, sync_cfg=storage_sync_cfg)
            ...
            if bool(confluence_cfg.get("enabled", True)):
                try:
                    ...
                    self._persist_ma10_macd_confluence_snapshot(
                        symbol,
                        timestamp=getattr(flow_snapshot, "timestamp", None),
                        confluence=confluence,
                        cfg=confluence_cfg,
                        sync_cfg=storage_sync_cfg,
                    )
                except Exception as e:
            ...
            if isinstance(position, dict):
                ...
                if current_side in ("LONG", "SHORT") and current_portion > 0 and decision.operation != FundFlowOperation.CLOSE:
                    ...
                    tf_15m = tf_ctx_all.get("15m", {}) if isinstance(tf_ctx_all, dict) else {}
                    tf_exec = tf_15m if isinstance(tf_15m, dict) and tf_15m else (
                        tf_ctx_all.get(self._execution_timeframe(), {}) if isinstance(tf_ctx_all, dict) else {}
                    )
                    tf_exec = tf_exec if isinstance(tf_exec, dict) else {}
                    ...
                    energy_now = max(
                        ...
                        min(
                            ...
                            + 0.20 * abs(0.15 * mtf_scores["1m"] + 0.25 * mtf_scores["3m"] + 0.60 * mtf_scores["15m"]),
                        ),
                    )

                    bb_upper_exec = self._to_float(tf_exec.get("bb_upper"), 0.0)
                    bb_lower_exec = self._to_float(tf_exec.get("bb_lower"), 0.0)
                    bb_middle_exec = self._to_float(
                        tf_exec.get("bb_middle"),
                        self._to_float(decision_md.get("ma10_exec"), self._to_float(decision_md.get("ma10_15m"), self._to_float(decision_md.get("ma10_5m"), 0.0))),
                    )
                    close_exec = self._to_float(
                        decision_md.get("last_close_exec"),
                        self._to_float(
                            decision_md.get("last_close_15m"),
                            self._to_float(tf_exec.get("last_close"), self._to_float(tf_exec.get("mid_price"), current_price))
                        )
                    )
                    trap_now = self._to_float(
                        flow_context.get("trap_score") if isinstance(flow_context, dict) else None,
                        self._to_float(tf_exec.get("trap_last"), 0.0),
                    )
            
                    # 获取KDJ J值
                    kdj_j_norm = self._to_float(decision_md.get("kdj_j_norm"), 0.0)
                    if abs(kdj_j_norm) < 1e-9:
                        kdj_j_raw = self._to_float(tf_exec.get("kdj_j"), 50.0)
                    ...
                    protection = self.risk_manager.check_position_protection(
                        ...
                        ma10_ltf=self._to_float(decision_md.get("ma10_exec"), self._to_float(decision_md.get("ma10_15m"), self._to_float(decision_md.get("ma10_5m"), 0.0))),
                        last_close=self._to_float(
                            decision_md.get("last_close_exec"),
                            self._to_float(decision_md.get("last_close_15m"), self._to_float(decision_md.get("last_close"), 0.0)),
                        ),
                        mtf_scores=mtf_scores,
                        energy=energy_now,
                        bb_upper=bb_upper_exec,
                        bb_lower=bb_lower_exec,
                        bb_middle=bb_middle_exec,
                        close_price=close_exec,
                        trap_score=trap_now,
                    ...
                    confluence_cfg_runtime = self._ma10_macd_confluence_config()
                    macd_mtf_guard = self._evaluate_macd_mtf_risk_guard(
                        current_side,
                        decision_md,
                        cfg=confluence_cfg_runtime,
                    )
                    decision_md["macd_mtf_risk_guard"] = bool(macd_mtf_guard.get("active", False))
                    decision_md["macd_mtf_risk_guard_detail"] = dict(macd_mtf_guard)
                    print(
                        "🧪 风控摘要 "
                    ...
                    if protection_level == "conflict_light":
                        ...
                        if bool(macd_mtf_guard.get("active", False)) and bool(
                            confluence_cfg_runtime.get("risk_guard_skip_conflict_tighten", True)
                        ):
                            allow_light_tighten = False
                        ...
                        if bool(macd_mtf_guard.get("active", False)) and bool(
                            confluence_cfg_runtime.get("risk_guard_skip_conflict_reduce", True)
                        ):
                            allow_light_take_profit = False
                        print(
                            ...
                            f"macd_mtf_guard={int(bool(macd_mtf_guard.get('active', False)))}"
                        )
