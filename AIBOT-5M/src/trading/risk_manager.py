...
class RiskManager:
    ...
    def _eval_minute_position_state(
        ...
        # 1) 多周期方向融合（1m/3m/15m，15m 为主趋势）
        tf_scores = mtf_scores if isinstance(mtf_scores, dict) else {}
        s1 = float(tf_scores.get("1m", 0.0))
        s3 = float(tf_scores.get("3m", 0.0))
        s15 = float(tf_scores.get("15m", tf_scores.get("5m", 0.0)))
        has_mtf = any(abs(v) > 1e-9 for v in (s1, s3, s15))
        dir_score = 0.15 * s1 + 0.25 * s3 + 0.60 * s15 if has_mtf else (0.65 * float(ev_score) + 0.35 * float(lw_score))
